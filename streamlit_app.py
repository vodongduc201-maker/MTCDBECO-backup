...
def main():
    ...
    for key, val in {
        ...
        "last_attempt_failed": False,     # Show retry button nếu write fail
    ...
    if st.session_state.confirm_pending:
        ...
        if all_ton_zero and st.session_state.het_hang_confirmed is None:
            ...
            with c_yes:
                ...
                if st.button("✅ Đúng, hết hàng → Gửi luôn", use_container_width=True,

                             type="primary", key="hh_yes_confirm"):

                    # Flow mới: Đúng = submit thẳng, bỏ qua bước Xác nhận

                    st.session_state.het_hang_confirmed  = True

                    st.session_state.is_submitting       = True

                    st.session_state.last_attempt_failed = False
        ...
        # Chỉ hiện nút "Xác nhận & Gửi" khi:

        # - KHÔNG phải case hết hàng, HOẶC

        # - Case hết hàng nhưng đang retry sau khi fail

        if (st.session_state.het_hang_confirmed is not True

                or st.session_state.last_attempt_failed):

            if (st.session_state.het_hang_confirmed is True

                    and st.session_state.last_attempt_failed):
        ...
        if st.session_state.is_submitting:
            ...
            if st.session_state.submitted_row_hash == row_hash:
                ...
                if safe_append_to_sheets(st.session_state.pending_row):
                    ...
                    # FIX 1: fail write → clear last_submit_time + bật cờ retry

                    st.session_state.is_submitting       = False

                    st.session_state.last_submit_time    = None

                    st.session_state.last_attempt_failed = True
