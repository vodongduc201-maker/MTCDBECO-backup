...
GIO_CHAN_H, GIO_CHAN_M = 17, 10   # 17:10 — sau giờ này không gửi (trừ CTY)
NGAY_CHAN = 21                    # Sau ngày 21 chỉ gửi hệ thống ưu tiên
MAX_LAN_PHU = 2                   # Tối đa 2 lần/tháng cho hệ thống phụ
MIN_WAIT_SECONDS = 360            # Chống spam — 6 phút

THUNG_LON = 24
THUNG_NHO = 12
SP_CHUA_1_5L = "1.5L"             # marker: chai 1.5L đóng thùng 12

SP_CHUAN = [                       # Thứ tự cột cố định (7 SP × 2 = 14 cột)
...
THU_LIST = ["Thu Hai", "Thu Ba", "Thu Tu", "Thu Nam",
...
DEFAULT_SESSION = {
    ...
    "last_submit_date":  None,   # dùng để reset chéo ngày
    "df_history":        None,
}
...
def render_progress(done, total, label=""):
    if total <= 0:
        st.markdown(
            f'<div class="prog-wrap"><div class="prog-info">'
            f'<span>{label}</span><span>0/0 (0%)</span></div></div>',
...
def load_data():
    ...
    if not nv_file:
        st.error(
            f"❌ Không tìm thấy file nhân viên. Đặt 1 trong: {NV_FILE_CANDIDATES}"
        )
    ...
    if not sp_file:
        st.error(
            f"❌ Không tìm thấy file danh mục. Đặt 1 trong: {SP_FILE_CANDIDATES}"
        )
        return pd.DataFrame(), pd.DataFrame()
...
def safe_append_to_sheets(flat_row):
    """Ghi Sheets với retry 3 lần — retry CẢ setup (creds, client, open)."""
    last_err = None
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                try:
                    st.toast(f"⚠️ Thử lại lần {attempt + 2}…")
                except Exception:
                    pass
...
def get_history_df(conn):
    try:
        ...
        if df.empty:
            return pd.DataFrame(
                columns=["NGAY", "THU", "GIO", "NHAN VIEN",
                         "HE THONG", "PHUONG", "SIEU THI"]
            )
        return df
    except Exception as e:
...
# ================================================================
# HELPERS — BLOCK CHECK (per-NV + history fallback)
# ================================================================
def _per_nv_key(nv):
    return f"last_submit_time__{nv}"


def check_block_status(sel_nv, today_str, now, df_history):
    """Trả về (is_blocked, wait_seconds). Per-NV để tránh khóa oan khi đổi NV."""
    # 1. Session per-NV (nhanh, 0 API)
    ...
    # 2. Fallback đọc từ history cache
    if df_history is not None and not df_history.empty:
        my_history = df_history[
            (df_history["NHAN VIEN"] == sel_nv) & (df_history["NGAY"] == today_str)
        ].copy()
        if not my_history.empty:
            my_history["DT"] = pd.to_datetime(
                my_history["NGAY"] + " " + my_history["GIO"],
...
def render_top_controls(df_master, df_history, sel_nv, now):
    """Chọn HT/ST + thống kê viếng thăm gần đây."""
    df_f1 = df_master[df_master["NHAN VIEN"] == sel_nv]
    sel_ht = st.selectbox(
        "🏪 Hệ thống", sorted(df_f1["HE THONG"].unique().tolist())
    )
    sel_st = st.selectbox(
        "🏬 Siêu thị",
    ...
    if not df_visits.empty:
        ...
        st.info(
            f"🕐 Viếng thăm gần nhất: **{last_str}** · "
            f"Tháng này: **{so_lan_thang} lần**"
        )
    else:
...
def build_flat_row(today_str, now, sel_nv, sel_ht, sel_st, phuong, inputs, note):
    """Build row theo schema cố định (7 cột meta + 14 cột SP + note + rỗng)."""
    flat = [
        today_str, THU_LIST[now.weekday()], now.strftime("%H:%M:%S"),
        sel_nv, sel_ht, phuong, sel_st,
    ]
    if sel_ht.upper() == "CTY":
...
def check_recent_duplicate(df_history, sel_nv, sel_st, today_str, sp_part):
    """Check trùng dữ liệu trong sheet để chống gửi 2 lần cùng nội dung."""
    ...
    if same.empty:
        return False, None
    # So sánh phần sản phẩm (14 cột SP) với row mới
    sp_cols = list(same.columns[7:7 + len(sp_part)])
    for _, row in same.iterrows():
        existing = [row.get(c) for c in sp_cols]
        if [None if pd.isna(v) else v for v in existing] == sp_part:
            return True, "Đã gửi dữ liệu giống hệt cho ST này trong ngày."
    return False, None
...
def render_confirm_screen(sel_nv, sel_ht, sel_st, today_str, now, inputs, note, df_history):
    ...
    sp_part = st.session_state.pending_row[7:-2]  # 14 cột SP
    ...
    if st.session_state.is_submitting:
        # Server-side dedup (phòng trường hợp session hash bị mất)
        is_dup, dup_msg = check_recent_duplicate(
            df_history, sel_nv, sel_st, today_str, sp_part
        )
        if is_dup and st.session_state.submitted_row_hash != row_hash:
            st.warning(f"⚠️ {dup_msg}")
        ...
        if st.session_state.submitted_row_hash == row_hash:
            ...
            # ✅ Set last_submit_time CHỈ sau khi ghi thành công
            st.session_state[_per_nv_key(sel_nv)] = now
            st.session_state.submitted_row_hash = row_hash
            ...
            for key in list(st.session_state.keys()):
                if key.startswith(("f_", "t_", "l_")):
                    del st.session_state[key]
            st.rerun()
        else:
            # Fail → reset submitting để user retry, KHÔNG set last_submit_time
            st.session_state.is_submitting = False
...
def main():
    st.set_page_config(
        page_title=f"Báo Cáo MT - {APP_VERSION}",
        page_icon="🥤",
        layout="centered",
    )
    ...
    if not df_history.empty and "NGAY" in df_history.columns:
        df_history["NGAY_DT"] = pd.to_datetime(
            df_history["NGAY"], format="%d/%m/%Y", errors="coerce"
        )
    else:
    ...
    if ht_up != "CTY" and (now.hour * 60 + now.minute) >= gio_chan_min:
        block_reasons.append(
            f"🌙 Đã sau {GIO_CHAN_H:02d}:{GIO_CHAN_M:02d}, không thể gửi."
        )

    if now.day > NGAY_CHAN and ht_up not in UU_TIEN_LIST:
        block_reasons.append(
            f"🚫 Sau ngày {NGAY_CHAN}, chỉ hệ thống ưu tiên được gửi."
        )
    ...
    if ht_up not in UU_TIEN_LIST and so_lan >= MAX_LAN_PHU:
        block_reasons.append(f"🚫 {sel_st} đã đủ {MAX_LAN_PHU} lần trong tháng.")

    # Block 6 phút (per-NV)
    is_waiting, wait_time = check_block_status(sel_nv, today_str, now, df_history)
    if is_waiting:
        st.error(
            f"🚫 Bạn vừa mới gửi báo cáo. "
            f"Vui lòng chờ {wait_time} giây để tiếp tục."
        )
    ...
    if st.session_state.confirm_pending:
        render_confirm_screen(
            sel_nv, sel_ht, sel_st, today_str, now,
            st.session_state.form_inputs,
            st.session_state.form_note,
            df_history,
        )
    ...
    if not is_blocked:
        if st.button(
            "🚀 GỬI BÁO CÁO", type="primary", use_container_width=True,
            disabled=st.session_state.confirm_pending,
        ):
            phuong_match = df_master[
                (df_master["NHAN VIEN"] == sel_nv)
                & (df_master["SIEU THI"] == sel_st)
            ]["PHUONG"].values
            p = phuong_match[0] if len(phuong_match) > 0 else ""

            flat_row = build_flat_row(
                today_str, now, sel_nv, sel_ht, sel_st, p, inputs, note
            )
